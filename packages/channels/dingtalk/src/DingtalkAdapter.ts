...
export class DingtalkChannel extends ChannelBase {
  ...
  private resolveSessionWebhook(chatId: string): string | undefined {
    const value = this.webhooks.get(chatId) ?? chatId;
    try {
  ...
  private async prepareFileOutput(
    ...
    if (streamed) {
      ...
      if (!streamed.matches(text)) {
        paths = [];
        if (projection.markerCount + streamedResult.markerCount > 0) {
          notices.push(
            '[File delivery failed: final response changed during streaming]',
          );
        }
      }
    }
    if (this.config.blockStreaming === 'on' && projection.markerCount > 0) {
      paths = [];
      notices.push('[File delivery failed: unavailable in block streaming]');
    }
    notices.push(...(await this.deliverFiles(paths, send)));
  ...
  private async sendReply(
    ...
    const outgoingText = prepared
      ? text
      : await this.prepareReplyOutput(webhook, text);
  ...
  override onSessionDied(sessionId: string): void {
    ...
    if (cardRunId) {
      ...
      this.deleteFileProjectorsForRun(cardRunId);
  ...
  protected override async onResponseComplete(
    ...
    const replyTarget = this.resolveSessionWebhook(chatId) ?? chatId;
    ...
    const outgoingText = await this.prepareReplyOutput(
      replyTarget,
      text,
      streamed,
    );
    ...
    const atUserId = this.atSender
      ...
      : undefined;
    if (atUserId) this.sessionMentionTargets.delete(sessionId);
    await this.sendReply(replyTarget, outgoingText, atUserId, true);
  }
